...
@onready var backgroundLists = $Main/Workspace/SidePanel/Data/BackgroundLists
...
func _run_tree(start_node):	
	...
	data.backgroundList = backgroundLists.filePath.text
...
func open_data(data : DialogueData):
	...
	backgroundLists.load_file(data.backgroundList)
...
func _on_node_added(node_name):
	...
	match( type ):
		...
		'1':
			...
			backgroundLists.file_selected.connect(node._on_backgrounds_loaded)
			node._on_characters_loaded(characters.characterList)
			node._on_backgrounds_loaded(backgroundLists.bgLists)
		'2':
