...
var backgroundLists : BackgroundLists = null
...
func set_data(new_data : DialogueData):
	...
	if dialogue_data:
		...
		# load characters
		characterList = null
		backgroundLists = null
		if dialogue_data.characters.ends_with('.tres'):
			var file = ResourceLoader.load(dialogue_data.characters, '', ResourceLoader.CACHE_MODE_REUSE)
			if file is CharacterList:
				characterList = file
				
		if dialogue_data.backgroundList.ends_with('.tres'):
			var file = ResourceLoader.load(dialogue_data.backgroundList, '', ResourceLoader.CACHE_MODE_REUSE)
			if file is BackgroundLists:
				backgroundLists = file
			else:
				print('somethigns fucked')
...
func _set_dialogue(dict):
	...
	if dict['speaker'] is String:
		...
		if idx > -1 and idx < characterList.characters.size():
			...
			var charStateIdx = dict['characterState']
			if charStateIdx != -1:
				characterState = characterList.characters[idx].states[charStateIdx]
			else:
				characterState = null
	
	if dict.has('backgroundList') and backgroundLists:
		var idx = dict['backgroundList']
		
		var backgrounds = backgroundLists.backgroundLists[idx].images
		if dict.has('background') and backgrounds:
...
func forceShowOptions():
	if !force_option_timer:
		force_option_timer = Timer.new()
		add_child(force_option_timer)
	else:
		force_option_timer.stop()
	
	force_option_timer.start(3)
	await force_option_timer.timeout
	options.show()
