...
@onready var backgroundList = $HBoxContainer5/BackgroundListSelect
@onready var background = $HBoxContainer3/BackgroundSelect
@onready var characterState = $HBoxContainer4/CharacterStateSelect
...
var curBackgroundList : int = -1
var curBackground : int = -1
var curCharacterState : int = -1

var availableBackgroundLists : Array[BackgroundList]
var availableCharacters : Array[Character]
...
func _to_dict(graph):
	...
	dict['backgroundList'] = backgroundList.selected
	dict['background'] = background.selected
	dict['characterState'] = characterState.selected
...
func _from_dict(graph, dict):
	...
	if dict.has('backgroundList') && dict['backgroundList'] is int:
		curBackgroundList = dict['backgroundList']

	if dict.has('background') && dict['background'] is int:
		curBackground = dict['background']
		
	if dict.has('characterState') && dict['characterState'] is int:
		curCharacterState = dict['characterState']
...
func _on_characters_loaded(newCharacterList : Array[Character]):
	...
	availableCharacters = newCharacterList
	
	for newCharacter in newCharacterList:
		speaker.add_item(newCharacter.editorName)
	
	if newCharacterList.size() > 0:
		if curSpeaker < newCharacterList.size():
			speaker.selected = curSpeaker
			characterState.clear()
			for charState in newCharacterList[curSpeaker].states:
				characterState.add_item(charState.name)
		else:
			curSpeaker = 0
			
		if curCharacterState < newCharacterList[curSpeaker].states.size():
			characterState.selected = curCharacterState
		else:
			curCharacterState = -1
		
			
func _on_backgrounds_loaded(newBackgroundLists : Array[BackgroundList]):
	backgroundList.clear()
	
	availableBackgroundLists = newBackgroundLists
	
	for newBackgroundList in newBackgroundLists:
		backgroundList.add_item(newBackgroundList.name)
	
	if newBackgroundLists.size() > 0:
		if curBackgroundList < newBackgroundLists.size():
			backgroundList.selected = curBackgroundList
			background.clear()
			for bg in newBackgroundLists[curBackgroundList].images:
				background.add_item(bg.name)
		else:
			curBackgroundList = -1
		
		if curBackground < newBackgroundLists[curBackgroundList].images.size():
			background.selected = curBackground
		else:
			curBackground = -1
...
func _on_speaker_selected(idx : int):
	...
	characterState.clear()
	var selectedChar = availableCharacters[idx]
	for state in selectedChar.states:
		characterState.add_item(state.name)
	characterState.select(-1)
	
	_on_node_modified()


func _on_background_list_select_item_selected(index):
	curBackgroundList = index
	
	background.clear()
	var selectedBgList = availableBackgroundLists[index]
	for bg in selectedBgList.images:
		background.add_item(bg.name)
...
func _on_reset_background_list_pressed():
	backgroundList.selected = -1
	background.selected = -1


func _on_reset_character_state_pressed():
	characterState.selected = -1
